...
class QuestionLocalDataSource {
  ...
  List<Map<String, dynamic>> _formatQuestions(List<Map> questions) {
    return questions.map((q) {
      ...
      return {
        ...
        'order': q[QuestionTable.displayOrder],
  ...
  _deocodeChoices(String choices) {
    return jsonDecode(choices);
  }

  // Get all questions inside a lesson
  Future<Map<String, dynamic>> getAllQuestionsByLesson(int lessonId) async {
    try {
      String query = '''
        SELECT q.${QuestionTable.id}, q.${QuestionTable.uuid}, 
               q.${QuestionTable.questionGroupId}, q.${QuestionTable.displayOrder} as order, 
               q.${QuestionTable.idTypeQuestion} as type_id, 
               q.${QuestionTable.textQuestion} as text_question, 
               q.${QuestionTable.questionPhoto}, q.${QuestionTable.choices}, 
               q.${QuestionTable.rightChoice} as right_choice, 
      ...
      return {
        QuestionsResponseModel.dataKey: formattedQuestions,
        QuestionsResponseModel.messageKey: 'Questions in Lesson $lessonId',
        QuestionsResponseModel.statusKey: 200,
      };
    } catch (e) {
      print('Error getting all questions by lesson: $e');
      return {'data': [], 'message': 'Error: $e', 'status': 500};
    }
  }
}
