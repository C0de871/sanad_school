...
class SubjectRepositoryImpl implements SubjectRepository {
  ...
  Future<Either<Failure, List<SubjectEntity>>> getSubjects({
    ...
    final subjectsFromLocal = await subjectLocalDataSource.getAllSubjects();
    ...
    if (subjectsFromLocal.isEmpty || isRefresh) {
      if (await networkInfo.isConnected!) {
        try {
          ...
          await subjectLocalDataSource.syncSubjectsAndTypes(response);
  ...
  @override
  Future<Either<Failure, bool>> downloadSubject(
    int subjectId, {
    bool isRefresh = false,
  }) async {
    try {
      final isSynced = await isSubjectSynced(
        CheckSubjectSyncParams(subjectId: subjectId),
      );
      return isSynced.fold((failure) => Left(failure), (isSynced) async {
        if (isSynced && !isRefresh) {
          final notLoadedQuestionsPhotoUrls = await questionLocalDataSource
              .getNotLoadedQuestionsPhotoUrlBySubjectId(subjectId: subjectId);
          return Right(true);
        } else {
          if (await networkInfo.isConnected!) {
            // 1. Fetch the subject data from the remote source
            final remoteSubject = await remoteDataSource.getSubjectSync(
              subjectId,
            );
            // 2. sync subject details with local database
            await subjectDetailLocalDataSource.syncSubjectDetails(
              remoteSubject,
            );
            return Right(true);
          } else {
  ...
  @override
  Future<Either<Failure, bool>> isSubjectSynced(
    CheckSubjectSyncParams params,
  ) async {
    try {
      final isSynced = await subjectLocalDataSource.isSubjectSynced(
        params.subjectId,
      );
